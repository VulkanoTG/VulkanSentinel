...
export function renderControlsInvertOverlayPage() {
  return `<!doctype html>
<html lang="pt-BR">
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>${escapeHtml("Sentinela // Inversao de Controles")}</title>
  ...
  <link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Material+Symbols+Outlined:opsz,wght,FILL,GRAD@20..48,100..700,0..1,-50..200&icon_names=keyboard_previous_language" />
  <style>
    :root {
      color-scheme: dark;
    ...
    * {
    ...
    .callout.is-active .frame::before {
      animation: lineReveal ${ENTRY_DURATION_MS}ms cubic-bezier(.16,.86,.22,1) forwards;
    }
    ...
    .callout.is-exiting .frame::before {
      animation: lineExit ${EXIT_DURATION_MS}ms cubic-bezier(.68,.06,.82,.24) forwards;
    }
    .panel {
      position: absolute;
      inset: 14px 0 auto;
    ...
    .callout.is-active .panel {
      animation: panelReveal ${ENTRY_DURATION_MS}ms cubic-bezier(.18,.84,.18,1) forwards;
    }
    ...
    .callout.is-active .timer {
      animation: contentReveal 360ms ease 150ms forwards;
    }
    ...
    .callout.is-exiting .timer {
      animation: contentExit 280ms cubic-bezier(.4,0,.2,1) forwards;
    }
    .icon {
...
<body>
  <main class="overlay-root">
    <section class="callout" id="overlay" aria-live="polite" aria-label="${escapeHtml("Overlay de inversao de controles")}">
      <div class="frame" aria-hidden="true"></div>
      <article class="panel">
        <div class="layout">
          <div class="icon-shell" aria-hidden="true">
            <span class="icon">keyboard_previous_language</span>
          </div>
          <div class="timer">
  ...
  <script>
    (() => {
      ...
      function playStartChime() {
        ...
        toneA.type = "triangle";
        toneA.frequency.setValueAtTime(720, startAt);
        toneA.frequency.exponentialRampToValueAtTime(980, startAt + 0.18);
        toneA.connect(gainNode);

        const toneB = audioContext.createOscillator();
        toneB.type = "sine";
        toneB.frequency.setValueAtTime(1040, startAt + 0.06);
        toneB.frequency.exponentialRampToValueAtTime(1320, startAt + 0.24);
      ...
      function connect() {
        const events = new EventSource("/api/overlays/controls-invert/stream");

        events.addEventListener("controls-invert", (event) => {
          try {
            ...
            console.error("[ControlsInvertOverlay] Falha ao processar payload:", error);
          }
        });
